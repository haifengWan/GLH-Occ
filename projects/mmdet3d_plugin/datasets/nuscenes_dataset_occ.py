    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        ...
        if metric == 'ray-iou':
            ...
            for i, batch in enumerate(data_loader):
                ...
                # Semantic RayIoU uses the standard Occ3D semantic labels.
                occ_gt_path = os.path.join(
                    info['occ_path'], 'labels.npz')
                ...
                result = occ_results[data_id]

                if isinstance(result, dict):
                    if 'pred_occ' not in result:
                        raise KeyError(
                            "RayIoU result dict has no 'pred_occ' key: "
                            f"{result.keys()}")
                    occ_pred = result['pred_occ']
                else:
                    occ_pred = result
                ...
                if torch.is_tensor(occ_pred):
                    ...
                    occ_pred = np.asarray(occ_pred)

                occ_pred = np.squeeze(occ_pred).astype(np.uint8)

                if occ_pred.shape != gt_semantics.shape:
                    raise ValueError(
                        f'Prediction shape {occ_pred.shape} does not match '
                        f'GT shape {gt_semantics.shape} for token {token}')

                lidar_origins.append(output_origin)
                occ_gts.append(gt_semantics)
                occ_preds.append(occ_pred)

                # Semantic occupancy predictions do not contain instance results.
                # Keep RayPQ support only when a panoptic model returns pano_inst.
                if (
                    isinstance(result, dict)
                    and 'pano_inst' in result
                    and 'instances' in occ_gt.files
                ):
                    pano_inst = result['pano_inst']

                    if torch.is_tensor(pano_inst):
                        pano_inst = pano_inst.detach().cpu().numpy()
                    else:
                        pano_inst = np.asarray(pano_inst)

                    pano_inst = np.squeeze(pano_inst).astype(np.int32)
