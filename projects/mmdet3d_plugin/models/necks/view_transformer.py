...
@NECKS.register_module(force=True)
class LSSViewTransformerSCGL(LSSViewTransformer):
    """
    SCGL: Semantic Channel-Grouped Lifting.

    Different channel groups use different depth distributions for lifting.
    It keeps the final BEV channel number unchanged.
    ...
    SCGL:
        depth_group: (B*N, G, D, H, W)
        feat_group:  split C into G groups
        bev = concat(BEVPool(depth_g, feat_g), dim=channel)
    """

    def __init__(
        self,
        scgl_num_groups=4,
        scgl_depth_fusion='mean',
        **kwargs
    ):
        super(LSSViewTransformerSCGL, self).__init__(**kwargs)

        assert self.out_channels % scgl_num_groups == 0, \
            'SCGL requires out_channels divisible by scgl_num_groups. ' \
            'Got out_channels={} and scgl_num_groups={}'.format(
                self.out_channels, scgl_num_groups
            )

        self.scgl_num_groups = scgl_num_groups
        self.scgl_depth_fusion = scgl_depth_fusion

        # Original depth_net output: D + C
        # SCGL depth_net output: G*D + C
        self.depth_net = nn.Conv2d(
            self.in_channels,
            self.D * self.scgl_num_groups + self.out_channels,
            kernel_size=1,
            padding=0
        )

    def view_transform_group_core(self, input, depth, tran_feat):
        """
        Group-wise view transformation.

        Args:
            depth:     (B*N, D, H, W)
            tran_feat: (B*N, Cg, H, W), where Cg = out_channels / G

        Returns:
            bev_feat:  (B, Cg, Dy, Dx)
        """
        B, N, _, H, W = input[0].shape
        group_channels = tran_feat.shape[1]

        if self.accelerate:
            feat = tran_feat.view(B, N, group_channels, H, W)
            ...
            # Keep same behavior as original accelerated branch.
            if self.collapse_z:
                if bev_feat.shape[2] == 1:
            ...
            bev_feat = self.voxel_pooling_v2(
                ...
                tran_feat.view(B, N, group_channels, H, W)
            )

        return bev_feat

    def view_transform_group(self, input, depth, tran_feat):
        if self.accelerate:
            self.pre_compute(input)
        return self.view_transform_group_core(input, depth, tran_feat)

    def forward(self, input):
        """
        Args:
            input[0]: (B, N, C_in, H, W)

        Returns:
            bev_feat: (B, out_channels, Dy, Dx)
            depth:    (B*N, D, H, W), group-averaged for compatibility
        """
        x = input[0]
        B, N, C, H, W = x.shape
        ...
        depth_logits = x[:, :self.D * self.scgl_num_groups, :, :]
        tran_feat = x[
            :,
            self.D * self.scgl_num_groups:
            self.D * self.scgl_num_groups + self.out_channels,
            :,
            :
        ]

        depth_logits = depth_logits.view(
            B * N,
            self.scgl_num_groups,
            self.D,
            H,
            W
        )

        feat_groups = torch.chunk(tran_feat, self.scgl_num_groups, dim=1)

        bev_groups = []
        depth_groups = []

        for g in range(self.scgl_num_groups):
            # Original code uses softmax(dim=1), not self.get_depth_dist().
            depth_g = depth_logits[:, g, :, :, :].softmax(dim=1).contiguous()
            feat_g = feat_groups[g].contiguous()

            bev_g = self.view_transform_group(input, depth_g, feat_g)

            bev_groups.append(bev_g)
            depth_groups.append(depth_g)

        bev_feat = torch.cat(bev_groups, dim=1)

        if self.scgl_depth_fusion == 'mean':
            depth = torch.stack(depth_groups, dim=1).mean(dim=1)
        elif self.scgl_depth_fusion == 'first':
            depth = depth_groups[0]
        else:
            raise ValueError(
                'Unsupported scgl_depth_fusion: {}'.format(
                    self.scgl_depth_fusion
                )
            )
...
@NECKS.register_module(force=True)
class LSSViewTransformerRSCGL(LSSViewTransformer):
    """
    R-SCGL: Residual Semantic Channel-Grouped Lifting.

    It keeps the original LSS depth distribution as the shared geometric prior,
    and learns group-specific residual depth logits for different feature
    channel groups.

    group_logits_g = base_logits + scale * tanh(delta_logits_g)

    At initialization, delta_logits_g = 0, so the module is exactly equivalent
    to the original LSS view transformer.
    """
    ...
    def __init__(
        ...
        scgl_num_groups=4,
        scgl_res_scale=0.2,
        scgl_depth_fusion='base',
        **kwargs
    ):
        super(LSSViewTransformerRSCGL, self).__init__(**kwargs)

        assert self.out_channels % scgl_num_groups == 0, \
            'R-SCGL requires out_channels divisible by scgl_num_groups. ' \
            'Got out_channels={} and groups={}'.format(
                self.out_channels, scgl_num_groups
            )

        self.scgl_num_groups = scgl_num_groups
        self.scgl_res_scale = scgl_res_scale
        self.scgl_depth_fusion = scgl_depth_fusion

        # Original depth_net is kept:
        # output = D + C
        #
        # Extra residual depth branch:
        # output = G * D
        self.group_depth_residual = nn.Conv2d(
            self.in_channels,
            self.scgl_num_groups * self.D,
            kernel_size=1,
            padding=0
        )
    ...
    def _init_residual_zero(self):
        nn.init.zeros_(self.group_depth_residual.weight)
        nn.init.zeros_(self.group_depth_residual.bias)

    def view_transform_group_core(self, input, depth, tran_feat):
        """
        Args:
            depth:     (B*N, D, H, W)
            tran_feat: (B*N, Cg, H, W)
    ...
    def forward(self, input):
        ...
        x_img = x.view(B * N, C, H, W)

        # Original LSS branch.
        base_out = self.depth_net(x_img)
        base_logits = base_out[:, :self.D, :, :]
        tran_feat = base_out[:, self.D:self.D + self.out_channels, :, :]

        # Residual grouped depth logits.
        delta_logits = self.group_depth_residual(x_img)
        delta_logits = delta_logits.view(
            B * N,
            self.scgl_num_groups,
            self.D,
            H,
        ...
        group_logits = base_logits.unsqueeze(1) + \
            self.scgl_res_scale * torch.tanh(delta_logits)
        ...
        for g in range(self.scgl_num_groups):
            depth_g = group_logits[:, g, :, :, :].softmax(dim=1).contiguous()
            feat_g = feat_groups[g].contiguous()
        ...
        base_depth = base_logits.softmax(dim=1)

        if self.scgl_depth_fusion == 'base':
            depth = base_depth
        elif self.scgl_depth_fusion == 'mean':
            depth = torch.stack(depth_groups, dim=1).mean(dim=1)
        elif self.scgl_depth_fusion == 'first':
            depth = depth_groups[0]
        else:
